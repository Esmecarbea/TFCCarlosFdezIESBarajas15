...
// Configuración del Pin 2 (ya existente)
const int pin2 = 2;
bool pin2State = false;

// Configuración del Pin 4 para Modo Taller
const int pin4 = 4;  // GPIO4 para Modo Taller
bool pin4State = false;  // Estado inicial del pin 4 (LOW)

void setupPin2() {
  pinMode(pin2, OUTPUT);
  digitalWrite(pin2, LOW);
  pin2State = false;
  Serial.println("Pin 2 configurado como salida y establecido en LOW");
}

// Nueva función para configurar el Pin 4
void setupPin4() {
  pinMode(pin4, OUTPUT);
  preferences.begin("my-app", true);  // Abrir en modo lectura
  pin4State = preferences.getBool("pin4State", false);  // Leer el estado, false por defecto
  preferences.end();
  digitalWrite(pin4, pin4State ? HIGH : LOW);  // Aplicar el estado leído
  Serial.printf("Pin 4 configurado como salida y establecido en %s\n", pin4State ? "HIGH" : "LOW");
}

// Activar Modo Taller (Pin 4 en HIGH)
void activateWorkshopMode() {
  if (!pin4State) {  // Solo cambiar si no está ya en HIGH
    digitalWrite(pin4, HIGH);
    pin4State = true;
    preferences.begin("my-app", false);  // Abrir namespace "my-app" en modo escritura
    preferences.putBool("pin4State", pin4State);  // Guardar el estado
    preferences.end();
    Serial.println("Modo Taller activado, Pin 4 establecido en HIGH");
  }
}

// Desactivar Modo Taller (Pin 4 en LOW)
void deactivateWorkshopMode() {
  if (pin4State) {  // Solo cambiar si no está ya en LOW
    digitalWrite(pin4, LOW);
    pin4State = false;
    preferences.begin("my-app", false);
    preferences.putBool("pin4State", pin4State);
    preferences.end();
    Serial.println("Modo Taller desactivado, Pin 4 establecido en LOW");
  }
}

// Resto del archivo (ra_filter_init, rgb_print, startCameraServer, etc.)
...
static int run_face_recognition(fb_data_t *fb, std::list<dl::detect::result_t> *results) {
  ...
  if (recognize.id >= 0) {
    ...
    if (!pin2State) {  // Si el pin no está ya en HIGH
      digitalWrite(pin2, HIGH);
      pin2State = true;
      Serial.println("Usuario registrado detectado, Pin 2 establecido en HIGH");
    }
  } else {
    rgb_print(fb, FACE_COLOR_RED, "Intruder Alert!");
    if (pin2State) {  // Si el pin no está ya en LOW
      digitalWrite(pin2, LOW);
      pin2State = false;
      Serial.println("Intruso detectado, Pin 2 establecido en LOW");
    }
  }
  return recognize.id;
}
...
void startCameraServer() {
  setupPin2();  // Inicializar el pin 2
  setupPin4();  // Inicializar el pin 4
